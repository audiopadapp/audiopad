...
#endif
{
    ...
    using namespace Audiopad::Globals; // NOLINT
    using namespace Audiopad::Objects; // NOLINT
    using namespace Audiopad::Enums;   // NOLINT
    ...
    {
        args.emplace_back(Audiopad::Helpers::narrow(arguments[i]));
    }
#else
...
#if defined(_WIN32)
    if (std::getenv("AUDIOPAD_DEBUG")) // NOLINT
    {
        AllocConsole();
...
#endif
    if (std::getenv("AUDIOPAD_DEBUG") != nullptr) // NOLINT
    {
    ...
    {
        Fancy::fancy.message() << "Audiopad usage" << std::endl;
    ...
    gGui = std::make_unique<Audiopad::Objects::WebView>();
    gGui->setup();
