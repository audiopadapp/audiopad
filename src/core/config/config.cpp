...
namespace Audiopad::Objects
{
    const std::string Config::path = []() -> std::string {
        ...
        if (configPath)
        {
            return std::string(configPath) + "/Audiopad/config.json";
        }
        return std::string(std::getenv("HOME")) + "/.config/Audiopad/config.json"; // NOLINT
#elif defined(_WIN32)
        char *buffer;
        std::size_t size;
        _dupenv_s(&buffer, &size, "APPDATA");
        auto rtn = std::string(buffer) + "\\Audiopad\\config.json";
        free(buffer);
